...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Load configuration
    let config = Arc::new(config::Config::load().unwrap_or_else(|_| {
        log!("Warning: Could not load config.toml, using defaults");
        config::Config::default()
    }));

    log!("Loaded configuration: dashboard_port={}, connection_port={}",
         config.network.dashboard_port, config.network.connection_port);

    // Extract values needed in async contexts
    let _dashboard_port = config.network.dashboard_port;
    let _connection_port = config.network.connection_port;
    ...
    // Bind connection listener (initial contact point)
    let bind_addr = if config.network.bind_host.is_empty() {
        format!("[::]:{}", config.network.connection_port)
    } else {
        format!("{}:{}", config.network.bind_host, config.network.connection_port)
    };
    let connection_listener = TcpListener::bind(&bind_addr).await?;
    log!("Connection listener listening on {} (port {})", bind_addr, config.network.connection_port);

    // Bind dashboard listener (plain HTTP for user interface)
    let dashboard_bind_addr = if config.network.bind_host.is_empty() {
        format!("[::]:{}", config.network.dashboard_port)
    } else {
        format!("{}:{}", config.network.bind_host, config.network.dashboard_port)
    };
    let dashboard_listener = TcpListener::bind(&dashboard_bind_addr).await?;
    log!("Dashboard listener listening on {} (port {})", dashboard_bind_addr, config.network.dashboard_port);

    // Bind secure peer listener (HTTPS for peer communication)
    let peer_bind_addr = if config.network.bind_host.is_empty() {
        format!("[::]:{}", 39002)
    } else {
        format!("{}:{}", config.network.bind_host, 39002)
    };
    let peer_listener = TcpListener::bind(&peer_bind_addr).await?;
    log!("Peer listener listening on {} (port 39002)", peer_bind_addr);
        ...
        for (peer_addr, peer_info) in peer_list {
            ...
            tokio::spawn(async move {
                ...
                let reconnect_success = match peer_client::connect_to_peer(&peer_ip, peer_port, true).await {
                    ...
                    Err(e) => {
                        log!("[STARTUP] ⚠ Failed to reconnect to {}: {}", peer_addr_clone, e.to_string());
                        false
                    }
                };
    ...
    let connection_port_broadcast = config.network.connection_port;
    let _broadcast_task = tokio::spawn(async move {
        if let Err(e) = broadcast::start_broadcast_listener(verified_peers_broadcast, connections_broadcast, connection_port_broadcast).await {
    ...
    let _connections_connection = connections.clone();
    ...
    // Spawn peer listener task
    let connections_for_peer = connections.clone();
    let config_for_peer = config.clone();
    let _shutdown_for_peer = shutdown.clone();
    let shutdown_for_main_loop = shutdown.clone();
    let _peer_task = tokio::spawn(async move {
        let acceptor_clone = acceptor.clone();
        loop {
            tokio::select! {
                result = peer_listener.accept() => {
                    match result {
                        Ok((stream, addr)) => {
                            let client_ip = addr.ip();
                            log!("[PEER] New peer connection from {} (port: {})", client_ip, addr.port());

                            // Clone config for this iteration
                            let _config = config_for_peer.clone();

                            // Upgrade the TCP connection to TLS
                            let acceptor = acceptor_clone.clone();
                            let connections_clone = connections_for_peer.clone();
                            let verified_peers_comm = verified_peers.clone();
                            tokio::spawn(async move {
                                log!("[TLS] Starting TLS handshake with peer {}", client_ip);

                                let tls_stream = acceptor.accept(stream).await;
                                match tls_stream {
                                    Ok(mut stream) => {
                                        log!("[TLS] ✓ Peer handshake successful with {}", client_ip);
                                        ...
                                        match stream.read(&mut buf).await {
                                            Ok(n) if n > 0 => {
                                                ...
                                                log!("[PEER] Request from {}: {}", client_ip, first_line);
                                                ...
                                                // Track connection
                                                {
                                                    ...
                                                    if is_new {
                                                        log!("[TRACK] Added new peer connection: {} (Total: {})", peer_key, conns.len());
                                                    } else {
                                                        log!("[TRACK] Updated peer connection: {} (Total: {})", peer_key, conns.len());
                                                    }
                                                }
                                                ...
                                                log!("[PEER] Sending 200 OK response for peer connection");
                                                if let Err(e) = stream.write_all(response.as_bytes()).await {
                                                    log!("[ERROR] Failed to write peer response: {:?}", e);
                                                } else {
                                                    log!("[PEER] ✓ Peer response sent successfully to {}", client_ip);
                                                }
                                            }
                                            Ok(_) => log!("[PEER] Connection closed by {}", client_ip),
                                            Err(e) => log!("[ERROR] Read error from peer {}: {:?}", client_ip, e),
                                        }
                                    }
                                    Err(e) => {
                                        log!("[TLS] ✗ Peer handshake failed with {}: {:?}", client_ip, e);
                        ...
                        Err(e) => log!("[ERROR] Peer listener error: {:?}", e),
                    }
                }
                _ = shutdown.notified() => {
                    log!("[SHUTDOWN] Peer listener shutting down");
                    break;
                }
            }
        }
    ...
    // Handle dashboard listener in main loop
    loop {
        tokio::select! {
            result = dashboard_listener.accept() => {
                match result {
                    Ok((mut stream, addr)) => {
                        let client_ip = addr.ip();
                        log!("[DASHBOARD] New connection from {} (port: {})", client_ip, addr.port());

                        // Handle plain HTTP requests for dashboard synchronously
                        let connections_clone = connections.clone();
                        ...
                        tokio::spawn(async move {
                            ...
                            match stream.read(&mut buf).await {
                                Ok(n) if n > 0 => {
                                    ...
                                    // Handle /api/settings endpoint for getting and updating configuration
                                    if path == "/api/settings" {
                                        log!("[API] Settings request from {}", client_ip);

                                        // Check if it's a POST request (update) or GET request (get current)
                                        let is_post = request.lines().next().unwrap_or("").to_uppercase().starts_with("POST");
                                        ...
                                        if is_post {
                                            ...
                                            let config_json = serde_json::to_string(&*config_clone).unwrap_or_else(|_| "{}".to_string());
                                    ...
                                    if path == "/broadcast" {
                                        ...
                                        // Send a broadcast message (synchronous)
                                        let broadcast_port = config_clone.network.connection_port;
                                        log!("[BROADCAST] Sending broadcast on port {}", broadcast_port);

                                        // Determine response message based on broadcast result
                                        let (response_body, _log_msg) = if broadcast::send_broadcast(broadcast_port).await.is_ok() {
                                            log!("[BROADCAST] ✓ Broadcast sent successfully on port {}", broadcast_port);
                                            (format!("Broadcast sent successfully on port {}", broadcast_port), "success")
                                        } else {
                                            log!("[BROADCAST] Error sending broadcast");
                                            ("Failed to send broadcast".to_string(), "error")
                                        };
                                    ...
                                    if path.starts_with("/connect") {
                                        ...
                                        if let Some(hostname) = hostname {
                                            if hostname.is_empty() {
                                                ...
                                                match tokio::net::lookup_host(format!("{}:{}", hostname, config_clone.network.dashboard_port)).await {
                                                    Ok(mut addrs) => {
                                                        if let Some(addr) = addrs.next() {
                                                            let peer_ip = addr.ip().to_string();
                                                            let peer_port = config_clone.network.dashboard_port;
                    ...
                    Err(e) => {
                        log!("[ERROR] Dashboard connection error: {:?}", e);
                    }
                }
            }
            _ = shutdown_for_main_loop.notified() => {
...
// Handle connection requests (initial contact point)
async fn handle_connection_request(
    ...
    log!("[CONNECTION] {} requested {}", client_ip, path);

    // Respond with dashboard port information (where P2P communication happens)
    ...
    log!("[CONNECTION] ✓ Sent dashboard port {} to {}", config.network.dashboard_port, client_ip);
    Ok(())
}
...
<body>
    ...
    <div class="nodes-section">
        ...
        <div id="settingsForm">
            <div class="setting-item">
                <label for="dashboardPort">Dashboard Port:</label>
                <input type="number" id="dashboardPort" min="1024" max="65535" class="search-box">
                <span class="setting-description">Port for dashboard and user interaction (HTTPS)</span>
            </div>
            <div class="setting-item">
                <label for="connectionPort">Connection Port:</label>
                <input type="number" id="connectionPort" min="1024" max="65535" class="search-box">
                <span class="setting-description">Port for initiating connections and broadcast messages</span>
            </div>
            <div class="controls">
                <button id="saveSettingsBtn" class="btn btn-primary">Save Settings</button>
                <button id="reloadSettingsBtn" class="btn btn-secondary">Reload Settings</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        const connectionPortInput = document.getElementById('connectionPort');
        ...
        function loadSettings() {{
            fetch('/api/settings')
                .then(response => response.json())
                .then(data => {{
                    if (dashboardPortInput) dashboardPortInput.value = data.network.dashboard_port;
                    if (connectionPortInput) connectionPortInput.value = data.network.connection_port;
        ...
        function saveSettings() {{
            const newConfig = {{
                network: {{
                    dashboard_port: parseInt(dashboardPortInput.value),
                    connection_port: parseInt(connectionPortInput.value),
                    bind_host: "" // Keep empty for now
                }},
                security: {{
            ...
            fetch('/api/settings', {{
                ...
                .then(data => {{
                    ...
                    alert('Settings saved successfully! You may need to restart the application for changes to take effect.');
                }})
                .catch(error => {{
