...
// Allow any client certificate (we'll verify via TOFU manually)
struct AllowAnyAuthenticatedClient;

impl rustls::server::ClientCertVerifier for AllowAnyAuthenticatedClient {
    fn client_auth_root_subjects(&self) -> Option<rustls::DistinguishedNames> {
        // Don't restrict which CAs can issue client certs
    ...
    fn verify_client_cert(
        ...
        // Accept any client certificate - we'll verify via TOFU after handshake
...
async fn handle_peer_connection(
    ...
    _verified_peers: Arc<RwLock<HashMap<String, PeerRecord>>>,
    connections: Arc<RwLock<HashMap<String, ConnectionInfo>>>,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create server config that REQUESTS client certificates (but doesn't require)
    // We'll verify peer certificates manually via TOFU after handshake
    ...
    // Get peer's certificate from the TLS session
    ...
    let peer_fingerprint = if let Some(cert_der) = peer_cert {
        ...
        log!("[PEER] ⚠ No client certificate presented by {}", client_ip);
        log!("[PEER] This is expected if peer is using a simple TLS client without mutual auth");
        // For now, we'll skip certificate verification if no cert is presented
        // The peer will still be tracked by IP
        return Ok(());
    };
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Shared state
    let _verified_peers: Arc<RwLock<HashMap<String, PeerRecord>>> = Arc::new(RwLock::new(HashMap::new()));
    ...
    let verified_peers_conn = _verified_peers.clone();
    ...
    let _shutdown_listener = shutdown.clone();
    let _verified_peers_listener: Arc<RwLock<HashMap<String, String>>> = Arc::new(RwLock::new(HashMap::new()));
